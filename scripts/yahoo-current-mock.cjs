...
const league={...require('../config/leagues/yahoo-example.json'),id:'yahoo-live-mock-check',platform:'manual',name:'Yahoo mock · current local build',targetTeam:'Marcus',teamCount:8,roster:{QB:1,WR:2,RB:2,TE:1,'R/W/T':1,K:1,DEF:1,BN:6}};
league.scoring.offense.reception=.5;league.scoring.offense.passingTouchdown=4;
const dir=path.resolve(__dirname,'../.media-build/yahoo-current-mock');fs.mkdirSync(dir,{recursive:true});
const app=buildApp({host:'127.0.0.1',port:0,instanceName:'CURRENT LOCAL BUILD · browser-assisted Yahoo mock',league,playerPool:{players:[],source:'Yahoo browser observations',complete:false},yahooOAuthEnabled:false,yahooDraftAutoSyncEnabled:false,fantasyProsSyncEnabled:false},{storeFactory:()=>new JsonStateStore(path.join(dir,'state.json')),yahooAccount:{status:()=>({connected:false})},yahooOAuth:{enabled:false,tokenStore:{configured:false}}});
