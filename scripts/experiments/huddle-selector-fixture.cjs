'use strict';
// Four-pick API/Huddle component fixture; owned picks 2/3 use 30/70 seconds.
// Screen-clock capture disabled. Source timestamps do not certify OCR.
...
const league={...require('../../config/leagues/yahoo-example.json'),id:'selector-validation',name:'Controlled browser validation',targetTeam:'Blitzkrieg',teamCount:2,roster:{QB:1,RB:1},provenance:{yahooLeagueKey:'999.l.123',yahooTeamKey:'999.l.123.t.2',verificationStatus:'verified'}};
...
const dir=path.resolve('.media-build/api-huddle-fixture-'+run);fs.mkdirSync(dir,{recursive:true});
let phase='waiting',deadline=0,due=0,autodraft=false,outageUntil=0;const picks=[],turns=[],browserEvents=[],streams=new Set();
const available=()=>players.filter(p=>!picks.some(x=>x.playerId===p.id));const mine=()=>pickOwner(picks.length+1,2)===2;
...
const service=app.draftService,session=service.createSession({draftSlot:2,sourceMode:'yahoo'});
function next(){deadline=Date.now()+(picks.length+1===3?70:30)*1000;due=Date.now()+750;if(mine())turns.push({pick:picks.length+1,start:Date.now(),deadline});}
function accept(player,manual){const pick=picks.length+1;if(mine())Object.assign(turns.at(-1),{selection:manual?'manual':'autodraft',playerId:player.id,acceptedAt:Date.now()});picks.push({pick,playerId:player.id,key:player.yahooPlayerKey,teamKey:'999.l.123.t.'+pickOwner(pick,2),at:Date.now()});if(picks.length===4)phase='completed';else next();}
...
const original=app.server.listeners('request')[0];app.server.removeAllListeners('request');app.server.on('request',async(req,res)=>{
 ...
 if(url.pathname==='/draftclient/f1/123/2'){
  res.writeHead(200,{'content-type':'text/html'});return res.end(fs.readFileSync(path.resolve('test/fixtures/selector-browser.html'),'utf8').replace('Math.ceil(state.pick/8)','Math.ceil(state.pick/2)'));
 }
...
app.server.on('close',()=>clearInterval(timer));app.server.listen(0,'127.0.0.1',()=>{const origin='http://127.0.0.1:'+app.server.address().port;const out={origin,room:origin+'/draftclient/f1/123/2',huddle:origin+'/draft-view.html?leagueId='+league.id+'&sessionId='+session.id,sessionId:session.id,leagueId:league.id,output:dir};fs.writeFileSync(path.join(dir,'launch.json'),JSON.stringify(out));console.log(JSON.stringify(out));app.yahooOperations.startDraftSync({leagueId:league.id,sessionId:session.id});});
