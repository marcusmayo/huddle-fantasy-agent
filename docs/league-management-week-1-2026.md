...
- Added a week-specific opponent panel using the opponent identified by Yahoo's scoreboard. The matchup summary stays visible, while the opponent's starters and bench are collapsed by default and can be opened for reference. The roster shows actual points, league-scored projections and expandable NFL matchup/news details. Starting-lineup totals exclude bench players. Saved weekly snapshots retain their own opponent roster; missing/legacy roster data is explicitly labelled.
- Combined the projected lineup and Yahoo player-results list into one team view above the opponent and waiver pool. Each player appears once with the Huddle suggested slot, Yahoo slot when different, actual points, weekly projection, and expandable matchup evidence. Bench and reserve players can be expanded separately. Completed weeks show Yahoo's played slots and actual points without retrospective forecasts.
...
Live verification added the actual Week 1 opponent roster to saved Week 1 revision 3 and the Week 2 opponent roster to saved Week 2 revision 4. Week 1 remains 245–188; the refreshed Week 2 current-starter projection comparison was 219.17 versus 207.15. Completed opponent views show actual final scores and hide retrospective projections. Fresh headlines are not attached to completed matchups as though they were historical news.
