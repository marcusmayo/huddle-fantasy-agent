...
# September 8 acceptance addendum

The latest live mock passed 15 manual picks, zero autopicks and all 120 reconciled results, with WR5/RB4/QB2/TE2/K1/DEF1. The [consolidated retrospective](draft-final-postmortem-2026-09-08.md) contains the full comparison and limitations.

`roster-contribution-v4-offensive-completion` reserves one real bench place for an owned quarterback beyond QB-eligible starting capacity when the league has room and does not explicitly opt into QB streaming. This produces two QBs for the standard mock and three for current DR Fantasy (two starting QBs). The model completes missing offensive starters in the latter half of roster construction before adding more depth, while keeping K/DEF late and allowing value to determine RB/WR depth. An exhausted QB pool produces a warning instead of blocking all candidates. Current DR settings are six teams, 15 starters, five bench, two IR: 20 drafted players and 120 total picks.

After acceptance, final-turn cleanup removes urgency and displays “No later turn” when no further own selection exists. The full suite then passed 183 tests. Automatic schedule/defense/injury/news feeds and complete raw-stat provider normalization remain unconnected; the sourced weekly-context contract below must not be described as an automatic production feed.
