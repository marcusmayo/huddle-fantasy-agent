...
function applyStatModifiers(scoring, rows) {
  ...
  for (const row of rows.filter((item) => Number.isFinite(item.value))) {
    ...
    const positionType = String(row.positionType || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
    const defensive = /defense|defensive|d st/.test(label) || ['d', 'dt', 'def', 'dst'].includes(positionType);
    let handled = true;
    ...
    else if (/offensive fumble return (touchdown|td)/.test(label)) scoring.offense.offensiveFumbleReturnTouchdown = row.value;
    else if (/return (touchdown|td)/.test(label) && !defensive) scoring.offense.returnTouchdown = row.value;
    ...
    else if (/field goals? 0 19|fg 0 19/.test(label)) scoring.kicking.fieldGoal0To19 = row.value;
    else if (/field goals? 20 29|fg 20 29/.test(label)) scoring.kicking.fieldGoal20To29 = row.value;
    else if (/field goals? 30 39|fg 30 39/.test(label)) scoring.kicking.fieldGoal30To39 = row.value;
    else if (/field goals? 40 49|fg 40 49/.test(label)) scoring.kicking.fieldGoal40To49 = row.value;
    else if (/field goals? 50|fg 50/.test(label)) scoring.kicking.fieldGoal50Plus = row.value;
    else if (/extra point.*made|point after attempt.*made|pat made/.test(label)) scoring.kicking.pointAfterAttemptMade = row.value;
    else if (/kickoff.*punt return (touchdown|td)|punt.*kickoff return (touchdown|td)/.test(label) && defensive) scoring.defense.kickoffOrPuntReturnTouchdown = row.value;
    else if (/extra point returned/.test(label) && defensive) scoring.defense.extraPointReturned = row.value;
    else if (/points allowed 0(?: points)?(?: |$)/.test(label) && defensive) scoring.defense.pointsAllowed['0'] = row.value;
    else if (/points allowed 1 6/.test(label) && defensive) scoring.defense.pointsAllowed['1-6'] = row.value;
    else if (/points allowed 7 13/.test(label) && defensive) scoring.defense.pointsAllowed['7-13'] = row.value;
    else if (/points allowed 14 20/.test(label) && defensive) scoring.defense.pointsAllowed['14-20'] = row.value;
    else if (/points allowed 21 27/.test(label) && defensive) scoring.defense.pointsAllowed['21-27'] = row.value;
    else if (/points allowed 28 34/.test(label) && defensive) scoring.defense.pointsAllowed['28-34'] = row.value;
    else if (/points allowed 35/.test(label) && defensive) scoring.defense.pointsAllowed['35+'] = row.value;
    ...
    else if (/block(ed)? kick/.test(label) && defensive) scoring.defense.blockedKick = row.value;
