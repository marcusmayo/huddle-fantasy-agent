...
function loadRuntimeConfig() {
  ...
  return {
    ...
    visualClockEnabled: parseBoolean(process.env.HUDDLE_VISUAL_CLOCK_ENABLED, false),
    ...
    yahooDraftPollIntervalMs: Math.max(5, Number(process.env.HUDDLE_YAHOO_DRAFT_POLL_SECONDS || 5)) * 1000,
    ...
    yahooWeeklyPlayerPageSize: Math.max(1, Math.min(100, Number(process.env.HUDDLE_YAHOO_WEEKLY_PLAYER_PAGE_SIZE || 100))),
    yahooWeeklyMaximumAvailablePlayers: Math.max(25, Math.min(1000, Number(process.env.HUDDLE_YAHOO_WEEKLY_MAX_AVAILABLE_PLAYERS || 500))),
    weeklyPersistedCandidateLimit: Math.max(5, Math.min(100, Number(process.env.HUDDLE_WEEKLY_PERSISTED_CANDIDATE_LIMIT || 25))),
    ...
    preflightYahooRehearsalEnabled: parseBoolean(process.env.HUDDLE_PREFLIGHT_YAHOO_REHEARSAL_ENABLED, true),
