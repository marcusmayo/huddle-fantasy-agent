...
function renderWeeklyOpponent(review) {
  ...
  const comparison = review.matchupComplete
    ? `Final score: your team ${review.targetResult?.score ?? 'unavailable'}; ${opponent.name} ${opponent.score ?? 'unavailable'}. Final results are shown; retrospective projections are hidden.`
    : `Current starting lineups: your team ${mine?.points ?? 'projection incomplete'}${mine?.points == null ? '' : ' projected points'}; ${opponent.name} ${theirs?.points ?? 'projection incomplete'}${theirs?.points == null ? '' : ' projected points'}. These are forecasts, not final scores.`;
  summary.textContent = `${comparison} Roster captured ${new Date(opponent.observedAt || review.observedAt).toLocaleString()}. Saved weeks retain that week’s opponent and roster.`;
